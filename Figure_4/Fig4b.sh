...
# Pattern
####
bart repmat 0 384 Fig4b_ptrn Fig4b_pattern
...
bart fmac k Fig4b_pattern k_usamp
...
# Undersample disentangled k-space
bart fmac k_ifftN Fig4b_pattern k_ifftN_usamp 
# Extract the slices
bart slice 2 0 k_ifftN_usamp k_ifftN_usamp_0
bart slice 2 1 k_ifftN_usamp k_ifftN_usamp_1   
# Calibration
bart ecalib -k3 -c0.2 -m1 k_ifftN_usamp_0 sens_usamp_0
bart ecalib -k3 -c0.2 -m1 k_ifftN_usamp_1 sens_usamp_1
...
# Slice ordering
bart flip 4 ESPIRiT_tmp ESPIRiT_4b
...
bart nlinv -H1 -S -i 10 -n1 k_usamp_NLINV rec_SMSNLINV_usamp
...
# Join into one file
bart join 4 rec_SMSNLINV_usamp_resize diff_SMSNLINVx5_resize_out SMSNLINV_4b


rm sens*.cfl k_*.cfl *pattern*.cfl rec*.cfl ESPIRiT_tmp.cfl diff*.cfl
rm sens*.hdr k_*.hdr *pattern*.hdr rec*.hdr ESPIRiT_tmp.hdr diff*.hdr
